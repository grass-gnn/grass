...
class MovePosToX(torch_geometric.transforms.BaseTransform):
...
class SuperpixelTrainer(BatchedNodeClassificationGRASSTrainer):
    def __init__(self, config: GRASSConfig) -> None:
        ...
        added_self_loop_attr = torch.tensor([0.0, 1.0, 0.0], dtype=torch.float16)
        added_random_edge_attr = torch.tensor([0.0, 0.0, 1.0], dtype=torch.float16)
